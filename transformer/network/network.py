...
class DNSModelBasic(nn.Module):
  def __init__(self, sample_rate, n_fft, frame, stride, device, batch):
    super(DNSModelBasic, self).__init__()
    ...
    # Values from Vaswani - Attention is all you need
    self.d_model = 512
    self.heads = 8
    self.enc = 6
    self.dec = 6
    ...
    self.linear_in = nn.Linear(self.n_fft*2, self.d_model)
    self.swish = nn.SiLU()
    ...
    self.linear_out = nn.Linear(self.d_model, self.n_fft*2)
  ...
  def forward(self, src, tgt, TRAIN=True):
    ...
    ## Source ##
    #print(src_spec.shape)
    src_spec = src_spec.permute(0,2,1,3)
    flattened_src = torch.flatten(src_spec, 2, 3)
    #print("flattened_src: ", flattened_src.shape)
    reshaped_src = flattened_src.view(-1, self.n_fft*2)
    ...
    if TRAIN:
      ...
      tgt_spec = tgt_spec.permute(0,2,1,3)
      flattened_tgt = torch.flatten(tgt_spec, 2, 3)
      #print("flattened_src: ", flattened_src.shape)
      reshaped_tgt = flattened_tgt.view(-1, self.n_fft*2)
      ...
      tgt_mask = self.get_tgt_mask(tgt_spec.shape[1]).to(self.device)
      emb_tgt = tgt.permute(1, 0, 2)
    ...
    lin_out = lin_out.view(tgt_spec.shape[1], self.batch, self.n_fft*2)
    lin_out = lin_out.view(tgt_spec.shape[1], self.batch, self.n_fft, 2)
    lin_out = lin_out.permute(1,2,0,3)
    #print("lin_out ", lin_out.shape)
...
class DNSModelCoeff(nn.Module):
  def __init__(self, sample_rate, n_fft, frame, stride, device, batch, POLAR=False):
    super(DNSModelCoeff, self).__init__()
    self.model_type = "DNSModelBasic"
    self.sample_rate = sample_rate
    self.n_fft = n_fft
    self.frame = frame
    ...
    self.fft_out_size = self.n_fft + 2
    if self.polar:
  ...
  def forward(self, src, tgt, TRAIN=True):
    ...
    if self.polar:
      ...
      lin_out = lin_out.view(tgt_spec.shape[1], self.batch, int(self.n_fft/2)+1, 2)
      lin_out = lin_out.permute(1,2,0,3)
    #print("lin_out ", lin_out.shape)
