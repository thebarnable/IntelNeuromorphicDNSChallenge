...
for epoch in range(epochs):
  ...
  if TRAIN:
    ...
    for i, (noisy, clean, noise) in enumerate(train_loader):
      ...
      if i == 0:
        saveAudio("debug2_coeffs_train_0_noisy_{}.wav".format(epoch), noisy[0], toCPU=True)
        saveAudio("debug2_coeffs_train_0_clean_{}.wav".format(epoch), clean[0], toCPU=True)
        saveAudio("debug2_coeffs_train_0_output_{}.wav".format(epoch), output[0].detach(), toCPU=True)
      if i == 1000:
        saveAudio("debug2_coeffs_train_1_noisy_{}.wav".format(epoch), noisy[0], toCPU=True)
        saveAudio("debug2_coeffs_train_1_clean_{}.wav".format(epoch), clean[0], toCPU=True)
        saveAudio("debug2_coeffs_train_1_output_{}.wav".format(epoch), output[0].detach(), toCPU=True)
      if i == 10000:
        saveAudio("debug2_coeffs_train_2_noisy_{}.wav".format(epoch), noisy[0], toCPU=True)
        saveAudio("debug2_coeffs_train_2_clean_{}.wav".format(epoch), clean[0], toCPU=True)
        saveAudio("debug2_coeffs_train_2_output_{}.wav".format(epoch), output[0].detach(), toCPU=True)
    print("Total loss: ", total_loss/len(train_loader))
    print("Time elapsed - train: ", datetime.now() - t_st)
  
    saveAudio("debug2_coeffs_train_3_noisy_{}.wav".format(epoch), noisy[0], toCPU=True)
    saveAudio("debug2_coeffs_train_3_clean_{}.wav".format(epoch), clean[0], toCPU=True)
    saveAudio("debug2_coeffs_train_3_output_{}.wav".format(epoch), output[0].detach(), toCPU=True)

    torch.save(net.state_dict(), "{}/model2_epoch_{}.pt".format(trained_folder, epoch))
    torch.save(dns_optimizer.state_dict(), "{}/optimizer2_epoch_{}.pt".format(trained_folder, epoch))
