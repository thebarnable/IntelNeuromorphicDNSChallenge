...
class Network(torch.nn.Module):
    def __init__(self,k,c,d,  threshold=0.1, tau_grad=0.1, scale_grad=0.8, max_delay=64, out_delay=0):
        ...
        self.blocks = torch.nn.ModuleList([
            torch.nn.Conv1d(2*257, c, kernel_size=k, padding='same', bias=False),
        ...
        self.blocks.append(torch.nn.Conv1d(c, 2*257, kernel_size=k, padding='same', bias=False))
        self.blocks.append(torch.nn.BatchNorm1d(2*257))
        self.blocks.append(torch.nn.ReLU(inplace=True))
...
if __name__ == '__main__':
    ...
    for kk,cc,dd in list(itertools.product(kernel_sizes,channel_sizes,nn_depth)):
        ...
        cfg_str = 'k' + str(kk) + 'c' + str(cc) + 'd' + str(dd) + '_optfcn' + '_Adam' + '_absarg'
        ...
        for epoch in range(max_epoch_per_run):
            t_st = datetime.now()
            for i, (noisy, clean, noise) in enumerate(train_loader):
                ...
                clean_absarg = torch.concat((clean_abs,clean_arg),dim=1)
                noisy_absarg = torch.concat((noisy_abs,noisy_arg),dim=1)

                denoised_absarg = net(noisy_absarg)

                denoised_abs = denoised_absarg[:,:noisy_abs.shape[1],:]
                denoised_arg = denoised_absarg[:,noisy_abs.shape[1]:,:]

                clean_rec = stft_mixer(denoised_abs, denoised_arg, args.n_fft)

                score = si_snr(clean_rec, clean)
                loss = lam * F.mse_loss(denoised_absarg, clean_absarg) + (100 - torch.mean(score))
