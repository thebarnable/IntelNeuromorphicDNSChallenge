...
if __name__ == '__main__':
    ...
    parser.add_argument("-hidden_input_ratio", type=float, default=2.0, help="Ratio of hidden layer and input layer size (only considered if transformation=mfcc, for stft ration is always 512/257)")
    ...
    parser.add_argument("-signal_quality_loss_type", type=str, default = "si-snr", choices = ["si-snr", "stoi"], help = "Which metric to use to measure signal quality")
    ...
    if args.signal_quality_loss_type == "stoi":
        print("Warning: STOI is extremely slow for use in the loss function")
        stoi = STOI(fs = 16000, extended = False)
        ...
        for i, (noisy, clean, noise) in enumerate(train_loader):
            ...
            if args.transformation == "stft":
                ...
                clean_rec = stft_mixer(
                    denoised_abs, noisy_arg, window, args.n_fft)
            ...
            elif args.signal_quality_loss_type == "stoi":
                signal_loss = stoi(noisy, clean) - stoi(clean_rec, clean)
            
            loss = mse_loss + signal_loss
            
            if torch.isnan(loss):
                print("WARNING: NaN detected in loss!!", loss)
            
            torch.nan_to_num(loss, nan=0, posinf = 0, neginf = 0)
